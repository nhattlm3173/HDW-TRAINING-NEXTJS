  ...
  return (
    <div className={`mx-1 my-3 flex w-fit flex-col space-y-1`}>
      <label className={`flex w-fit items-center space-x-2`}>
        <input
          type="checkbox"
          className={`h-5 w-5 rounded border-gray-300 text-indigo-600 ${error ? 'border-red-500 focus:ring-red-500' : ''} cursor-pointer disabled:cursor-not-allowed disabled:border-gray-200 disabled:bg-gray-100 ${className}`}
          {...props}
        />
        <span
